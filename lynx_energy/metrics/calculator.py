"""Energy-specialized metrics calculation engine.
...
def calc_energy_quality(
    ...
    if revenues and stage == CompanyStage.PRODUCER:
        ...
        m.revenue_predictability = "Early exploration — no revenue expected"
        score += 2
...
def calc_market_intelligence(
    ...
    _COMMODITY_TICKERS = {
        Commodity.CRUDE_OIL: ("CL=F", "WTI Crude Oil"),
        Commodity.NATURAL_GAS: ("NG=F", "Natural Gas"),
        Commodity.LNG: ("NG=F", "Natural Gas (LNG proxy)"),
        Commodity.NGL: ("CL=F", "Crude Oil (NGL proxy)"),
        Commodity.URANIUM: ("URA", "Uranium (ETF proxy)"),
        Commodity.COAL: ("BTU", "Coal (Peabody proxy)"),
    }
    _SECTOR_ETFS = {
        Commodity.CRUDE_OIL: [("XLE", "Energy Select Sector SPDR"), ("XOP", "SPDR S&P Oil & Gas Exploration")],
        Commodity.NATURAL_GAS: [("XLE", "Energy Select Sector SPDR"), ("FCG", "First Trust Natural Gas")],
        Commodity.LNG: [("XLE", "Energy Select Sector SPDR"), ("FCG", "First Trust Natural Gas")],
        Commodity.NGL: [("XLE", "Energy Select Sector SPDR"), ("XOP", "SPDR S&P Oil & Gas Exploration")],
        Commodity.URANIUM: [("URA", "Global X Uranium ETF"), ("URNM", "Sprott Uranium Miners")],
        Commodity.COAL: [("XLE", "Energy Select Sector SPDR"), ("KOL", "VanEck Coal ETF")],
        Commodity.HYDROGEN: [("HDRO", "Defiance Hydrogen ETF"), ("ICLN", "iShares Clean Energy")],
        Commodity.RENEWABLE: [("ICLN", "iShares Clean Energy"), ("TAN", "Invesco Solar ETF")],
        Commodity.OTHER: [("XLE", "Energy Select Sector SPDR"), ("IYE", "iShares U.S. Energy")],
    }

    # Detect commodity from the company profile
    company_commodity = Commodity.OTHER
    try:
        from lynx_energy.models import classify_commodity
    ...
    # ── 6. Projected dilution (pre-revenue energy companies) ─────────
    ...
    # ── 8. Energy disclaimers ────────────────────────────────────────
    disclaimers: list[str] = [
        "Energy investments carry significant commodity price risk. Oil & gas prices are highly cyclical.",
        "Reserve estimates are engineering opinions subject to revision as new data becomes available.",
        "Commodity prices are cyclical — current margins may not persist through the full cycle.",
    ]
    ...
    if stage == CompanyStage.DEVELOPER:
        disclaimers.append(
            "Development cost overruns of 20-40% above initial estimates "
            "are common in energy projects."
        )
    disclaimers.extend([
