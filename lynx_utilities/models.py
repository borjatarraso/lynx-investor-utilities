"""Data models for Lynx Utilities — utilities-focused fundamental analysis."""
...
# ---------------------------------------------------------------------------
# Utility company stage classification
#
# The enum *keys* are kept identical to the shared suite schema (GRASSROOTS,
# EXPLORER, DEVELOPER, PRODUCER, ROYALTY) so relevance tables and downstream
# logic stay compatible. Only the display labels change to utilities-appropriate
# terminology.
# ---------------------------------------------------------------------------

class CompanyStage(str, Enum):
    GRASSROOTS = "Early-Stage Developer"          # greenfield renewable / early project dev
    EXPLORER = "Development-Stage Utility"        # permitted / financing projects, pre-construction
    DEVELOPER = "Pre-Operational / Construction"  # project under construction, nearing COD
    PRODUCER = "Operating Utility"                # mature regulated or merchant utility
    ROYALTY = "YieldCo / Holding"                 # pure-yield holding or YieldCo structure


class Commodity(str, Enum):
    """Primary service/revenue type for utility companies.

    The name is kept as ``Commodity`` for schema parity with the rest of the
    Lince suite; the members describe utility service types and generation
    mix classifications rather than raw commodities.
    """
    REGULATED_ELECTRIC = "Regulated Electric"
    MERCHANT_POWER = "Merchant / Independent Power"
    ...
    DIVERSIFIED_UTILITY = "Diversified Utility"
    OTHER = "Other"


class JurisdictionTier(str, Enum):
    TIER_1 = "Tier 1 — Constructive Regulation"
    TIER_2 = "Tier 2 — Balanced / Mixed Regulation"
    TIER_3 = "Tier 3 — Challenging Regulation"
    UNKNOWN = "Unknown"
...
class ValuationMetrics:
    ...
    ev_per_boe: Optional[float] = None             # kept for schema parity (unused in utilities)
    ev_per_mcfe: Optional[float] = None            # kept for schema parity (unused in utilities)
    p_nav: Optional[float] = None                  # interpreted as P / Rate Base proxy for utilities
    cash_to_market_cap: Optional[float] = None
    nav_per_share: Optional[float] = None          # interpreted as Rate Base per Share for utilities
    fcf_yield: Optional[float] = None
    ev_per_daily_boe: Optional[float] = None       # kept for schema parity (unused in utilities)
...
class ProfitabilityMetrics:
    ...
    # The netback_* and operating_cost_per_boe fields are preserved for schema
    # parity with other Lince agents, but are not populated for utilities.
    netback_per_boe: Optional[float] = None
    netback_unit: str = "mwh"                      # default unit for utilities context
...
class GrowthMetrics:
    ...
    production_growth_yoy: Optional[float] = None     # interpreted as MW-capacity growth for utilities
...
class EnergyQualityIndicators:
    """Utilities quality indicators (name kept for schema parity across the
    Lince suite). Fields are interpreted against a utilities framework:
    ``reserve_quality`` stands in for rate-base quality, ``reserve_life_assessment``
    captures generation-asset longevity, ``production_scale_assessment`` maps to
    MW-scale / customer-count assessment, etc.
    """
    ...
    reserve_quality: Optional[str] = None                   # rate-base quality for utilities
    reserve_life_assessment: Optional[str] = None           # asset-life / useful life assessment
    production_scale_assessment: Optional[str] = None       # MW / customer scale assessment
...
class IntrinsicValue:
    ...
    ev_reserve_implied_price: Optional[float] = None         # kept for schema parity
...
class MarketIntelligence:
    """Market sentiment, insider activity, institutional holdings, and
    technicals. For utilities, benchmark context tracks sector ETFs (XLU,
    VPU) and front-of-curve power / natural gas prices — the latter drives
    margins for merchant / IPP names.
    """
    # Insider activity
    ...
    # Projected dilution (mainly relevant for pre-operational developers)
    ...
    # Market context — repurposed for utilities: the "commodity" slot carries
    # the most relevant front-of-curve input fuel / power price benchmark.
    ...
    # Utilities-specific disclaimers
...
class FinancialStatement:
    ...
    # Kept for schema parity with other Lince agents; unused in utilities:
...
class AnalysisReport:
    ...
    energy_quality: Optional[EnergyQualityIndicators] = None     # kept name for schema parity
...
_STAGE_KEYWORDS = {
    CompanyStage.PRODUCER: [
        "regulated utility", "integrated utility", "operating utility",
        "rate base", "allowed roe", "rate case", "customer base",
        "electric distribution", "gas distribution", "transmission and distribution",
        "water utility", "independent power producer", "ipp",
        "power generation", "operating capacity", "installed capacity",
        "generation fleet", "combined-cycle", "ccgt", "nuclear fleet",
        "generation and transmission", "merchant generation",
    ],
    CompanyStage.DEVELOPER: [
        "under construction", "in construction", "construction phase",
        "project commissioning", "pre-operational", "final investment decision",
        "ntp issued", "notice to proceed", "commercial operation date",
        "cod targeted", "pre-commercial", "construction spend",
    ],
    CompanyStage.EXPLORER: [
        "project pipeline", "development pipeline", "advanced development",
        "interconnection queue", "permitted projects", "fully permitted",
        "ppa signed", "offtake agreement signed", "pre-construction",
        "financial close", "ready-to-build", "ptc awarded", "itc awarded",
    ],
    CompanyStage.GRASSROOTS: [
        "early stage development", "greenfield", "site identification",
        "land banking", "resource assessment", "wind resource study",
        "solar resource study", "site control", "pre-permitting",
        "concept stage", "feasibility stage",
    ],
}

_COMMODITY_KEYWORDS = {
    Commodity.REGULATED_ELECTRIC: [
        "regulated electric", "electric utility", "integrated electric",
        "electric distribution", "electric transmission", "investor-owned utility",
    ],
    Commodity.MERCHANT_POWER: [
        "merchant power", "independent power producer", "ipp",
        "competitive generation", "wholesale power", "merchant generation",
    ],
    Commodity.REGULATED_GAS: [
        "natural gas utility", "gas distribution", "ldc",
        "local distribution company", "regulated gas",
    ],
    Commodity.MULTI_UTILITY: [
        "multi-utility", "multiutility", "combined utility",
        "diversified energy utility", "integrated energy services",
    ],
    Commodity.WATER: [
        "water utility", "water and wastewater", "wastewater",
        "water services", "water infrastructure",
    ],
    Commodity.RENEWABLE_POWER: [
        "renewable power", "wind power", "solar power", "solar farm",
        "wind farm", "utility-scale solar", "utility-scale wind",
        "renewable energy", "clean energy", "hydroelectric", "geothermal",
    ],
    Commodity.NUCLEAR_GENERATION: [
        "nuclear generation", "nuclear power", "nuclear fleet",
        "nuclear reactor", "nuclear plant",
    ],
    Commodity.TRANSMISSION_DISTRIBUTION: [
        "transmission company", "transmission and distribution",
        "t&d", "power grid", "grid operator", "grid services",
    ],
    Commodity.DIVERSIFIED_UTILITY: [
        "diversified utility", "global utility", "yieldco",
        "yield co", "utility holding",
    ],
}

# Regulatory jurisdiction tiers for utilities.
# Tier 1: historically constructive regulators with timely cost recovery.
# Tier 2: mixed / balanced regulatory environments.
# Tier 3 (fallback) covers less predictable regimes.
_TIER_1_JURISDICTIONS = {
    "united states", "usa", "us",
    "florida", "texas", "georgia", "alabama", "north carolina",
    "south carolina", "virginia", "indiana", "wisconsin", "iowa",
    "minnesota", "arizona",
...
_TIER_2_JURISDICTIONS = {
    "california", "new york", "new jersey", "illinois", "ohio",
    "massachusetts", "connecticut", "michigan", "pennsylvania",
    "oregon", "washington",
    "spain", "portugal", "italy", "france", "belgium", "austria",
    "ireland", "japan", "south korea", "chile",
...
def classify_stage(description: Optional[str], revenue: Optional[float],
    ...
    if revenue is not None and revenue > 50_000_000:
        import re as _re
        for pattern in [r"\byieldco\b", r"\byield co\b", r"\bholding company\b"]:
    ...
    if info:
        ...
        if "utilit" in industry or "utilit" in sector or "power" in industry:
...
def classify_jurisdiction(country: Optional[str],
    ...
    # Use word-boundary for description to avoid false positives.
