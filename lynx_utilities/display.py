"""Rich console display for utilities analysis reports — stage and tier aware.
...
# ---------------------------------------------------------------------------
# Assessment functions — all return meaningful text for utilities context
...
def _a_pe(val):
    ...
    if val < 0:
        return "[red]*WARNING* Negative earnings — common at pre-operational utility developers[/]"
    if val < 8:
    ...
    if val < 18:
        return "[green]\\[OK] Reasonably valued for an operating utility[/]"
    if val < 25:
        return "[yellow]\\[WATCH] Fair — pricing in production growth[/]"
    if val < 40:
        return "[#ff8800]*WARNING* Expensive — needs strong production catalysts[/]"
    return "[bold red]***CRITICAL*** Very expensive — cycle peak or speculative premium[/]"
...
def _a_pb(val):
    ...
    if val < 1.5:
        return "[green]\\[OK] Near book value — meaningful proxy for P/Rate Base at a utility[/]"
    if val < 2.5:
...
def _a_ps(val):
    ...
    return "[bold red]***CRITICAL*** Very expensive on revenue basis[/]"
...
def _a_pfcf(val):
    ...
    if val < 0:
        return "[bold red]***CRITICAL*** Negative FCF — burning cash or heavy capex cycle[/]"
    if val < 8:
        return "[dim]\\[STRONG] Strong FCF yield — harvest-mode utility[/]"
    if val < 15:
...
def _a_evrev(val):
    ...
    if val < 3.0:
        return "[yellow]\\[WATCH] Fair for a utility[/]"
    if val < 6.0:
...
def _a_divy(val):
    if _isna(val):
        return "[dim]No dividend — atypical; may indicate a pre-operational utility developer[/]"
    if val == 0:
        return "[dim]No dividend paid[/]"
    if val > 0.06:
        return "[dim]\\[STRONG] High yield — verify sustainability with FCF[/]"
    if val > 0.03:
        return "[green]\\[OK] Solid dividend yield for a utility[/]"
    if val > 0.01:
...
def _a_roe(val):
    ...
    if val < 0.12:
        return "[green]\\[OK] Adequate ROE for a utility vs allowed ROE[/]"
    if val < 0.20:
        return "[green]\\[OK] Strong ROE — efficient capital deployment[/]"
    return "[dim]\\[STRONG] Exceptional ROE — verify sustainability[/]"
...
def _a_roa(val):
    ...
    if val < 0.08:
        return "[green]\\[OK] Good ROA for capital-intensive utility[/]"
...
def _a_roic(val):
    ...
    return "[dim]\\[STRONG] Exceptional ROIC — world-class asset[/]"
...
def _a_gm(val):
    ...
    if val < 0.40:
        return "[yellow]\\[WATCH] Adequate gross margin for a utility (pass-through fuel may distort)[/]"
    if val < 0.55:
        return "[green]\\[OK] Strong margins — low-cost producer[/]"
    return "[dim]\\[STRONG] Exceptional — best-in-class cost structure[/]"
...
def _a_om(val):
    ...
    if val < -0.50:
        return "[bold red]***CRITICAL*** Heavy operating losses — explorer burn rate[/]"
    if val < 0:
    ...
    return "[dim]\\[STRONG] High operating leverage — efficient operations[/]"
...
def _a_nm(val):
    ...
    if val < -0.50:
        return "[bold red]***CRITICAL*** Deep losses — verify cash runway[/]"
    if val < 0:
        return "[#ff8800]*WARNING* Net loss — expected at pre-operational utility developers[/]"
    if val < 0.05:
        return "[yellow]\\[WATCH] Thin net margin — leverage and tax impact[/]"
    if val < 0.15:
        return "[green]\\[OK] Healthy net margin for a utility[/]"
    return "[dim]\\[STRONG] Strong profitability — low-cost advantage[/]"
...
def _a_ebitdam(val):
    ...
    return "[dim]\\[STRONG] Excellent — high operating leverage[/]"
...
def _a_de(val):
    ...
    if val < 0.50:
        return "[green]\\[OK] Moderate leverage — manageable for a utility (sector is structurally levered)[/]"
    if val < 1.00:
...
def _a_ic(val):
    ...
    return "[dim]\\[STRONG] Very strong — interest is trivial[/]"
...
def _a_revg(val):
    ...
    if val > 0.50:
        return "[dim]\\[STRONG] Exceptional revenue growth >50%[/]"
    if val > 0.20:
    ...
    return "[bold red]***CRITICAL*** Significant revenue decline[/]"
...
def _a_dil(val):
    ...
    if val < -0.05:
        return "[dim]\\[STRONG] Meaningful buybacks — shareholder friendly[/]"
    if val < -0.01:
...
def _a_shares_out(val):
    ...
    if val > 1_000_000_000:
        return "[bold red]***CRITICAL*** Over 1B shares — heavily diluted, limits upside per share[/]"
    if val > 500_000_000:
...
def _a_insider(val):
    ...
    return "[bold red]***CRITICAL*** Minimal insider ownership — check for pay-to-play[/]"
...
# --- Utilities-specific metric assessments ---
...
def _a_capex_rev(val):
    ...
    if val > 0.50:
        return "[bold red]***CRITICAL*** Capex >50% of revenue — extreme capital intensity[/]"
    if val > 0.35:
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Utilities Analysis[/]",
        border_style="blue",
    ))
...
def _display_energy_quality(report):
    """Utilities quality assessment — uses relevance markers via get_relevance with category='energy_quality'."""
    ...
    t = Table(title="Utilities Quality Assessment", show_lines=True, border_style="bold yellow")
    t.add_column("Indicator", style="bold", min_width=22, no_wrap=True)
    t.add_column("Assessment", ratio=1, overflow="fold")
    t.add_column("Impact", justify="center", min_width=12, no_wrap=True)
...
def _add_quality_row(table, label, value, relevance):
    """Add a row to the utilities quality table with relevance-based styling."""
    if relevance == Relevance.IRRELEVANT:
        return
    style, prefix = _STYLE.get(relevance, _STYLE[Relevance.RELEVANT])
    impact = _IMPACT.get(relevance, "[dim]—[/]")
...
def _display_screening(c):
    ...
    t = Table(title="Utilities Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    if mi.disclaimers:
        console.print(Panel(
            ...
            title="[dim]Utilities Investment Disclaimers[/]",
            border_style="dim",
        ))
