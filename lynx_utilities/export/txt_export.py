...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Utilities  --  {p.name} ({p.ticker})")
    lines.append(f"  Tier: {_ev(p.tier)}  |  Stage: {_ev(p.stage)}")
    lines.append(f"  Service Type: {_ev(p.primary_commodity)}  |  Jurisdiction: {_ev(p.jurisdiction_tier)}")
    ...
    if report.intrinsic_value:
        ...
        rows = [
            ...
            _iv_row("EV / Reserve Implied (legacy)", iv.ev_reserve_implied_price, None),
        ]
        for r in rows:
            if r is not None:
                lines.append(r)
    ...
    if report.market_intelligence:
        ...
        if mi.commodity_name or mi.sector_etf_name:
            ...
            if mi.commodity_name and mi.commodity_price:
                lines.append(_row("Market Benchmark", f"{mi.commodity_name} -- ${mi.commodity_price:,.2f}"))
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Utilities Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Utilities Analysis (Lince Investor Suite)")
    lines.append("=" * W)
