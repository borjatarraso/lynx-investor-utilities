"""Textual UI for Lynx Utilities Analysis."""
...
class ReportView(VerticalScroll):
    ...
    def add_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        if stage == "profile":
            ...
            self._mount_section("Utilities Quality", _build_energy_quality(report))
...
class LynxUtilitiesApp(PagingAppMixin, App):
    TITLE = "Lynx Utilities Analysis"
    SUB_TITLE = "Utilities Sector Analysis"
    CSS = """
    #search-dialog {
        width: 60;
        height: auto;
    ...
    def on_mount(self) -> None:
        from lynx_utilities.tui.themes import register_all_themes, THEME_NAMES
        try:
            register_all_themes(self)
            self.theme = THEME_NAMES[0]  # utilities-dark
        except Exception:
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Static(
            "[bold blue]  lynx-utilities  [/]\n\n"
            "[bold]Utilities Sector Analysis Tool[/]\n"
    ...
    def action_cycle_theme(self) -> None:
        from lynx_utilities.tui.themes import THEME_NAMES
    ...
    def action__ee_lynx(self) -> None:
        from lynx_utilities.easter import LYNX_ASCII, WOLF_ASCII, BULL_ASCII
        import random
        ...
        try:
            ...
            if isinstance(e, SectorMismatchError):
                self.call_from_thread(
                    ...
                    f"[red]Regulated Electric | Gas | Water | Multi-Utilities | IPPs | YieldCos[/]\n\n"
...
def _build_energy_quality(r: AnalysisReport) -> Static:
    """Build a Rich Table for utilities quality with multi-line cell wrapping."""
    m = r.energy_quality
    stage = _get_stage(r)
    tier = _get_tier(r)
    t = RichTable(show_lines=True, border_style="yellow", expand=True)
