"""Main analysis orchestrator for utility companies."""
...
from lynx_utilities.core.fetcher import fetch_company_profile, fetch_financial_statements, fetch_info
from lynx_utilities.core.news import fetch_all_news
from lynx_utilities.core.reports import download_top_filings, fetch_sec_filings
from lynx_utilities.core.storage import get_cache_age_hours, has_cache, load_cached_report, save_analysis_report
from lynx_utilities.core.ticker import resolve_identifier
...
_ENERGY_DESCRIPTION_PATTERNS = [
    r"\bregulated (electric|gas|water) utility\b",
    r"\belectric utility\b", r"\bgas utility\b", r"\bwater utility\b",
    r"\binvestor-owned utility\b",
    r"\bintegrated electric\b", r"\bmulti-utility\b",
    r"\bindependent power producer\b", r"\bipp\b",
    r"\bmerchant (power|generation)\b",
    r"\brate base\b", r"\ballowed roe\b", r"\brate case\b",
    r"\bpublic service commission\b", r"\bpuc\b",
    r"\bpower purchase agreement\b", r"\bppa\b",
    r"\bwholesale power\b", r"\bgeneration fleet\b",
    r"\btransmission and distribution\b", r"\bt&d\b",
    r"\byieldco\b", r"\byield co\b",
    r"\bwater and wastewater\b",
    r"\butility-scale (solar|wind)\b",
    r"\bnuclear (generation|fleet|plant|reactor)\b",
    r"\bcapacity factor\b", r"\binstalled capacity\b",
]

_SCOPE_MSG = (
    "the scope of this tool.\n\n"
    "Lynx Utilities Analysis is specialized exclusively for:\n"
    "  - Utilities (Regulated Electric, Gas, Water, Multi-Utilities, IPPs, YieldCos)\n\n"
...
_VALIDATOR = SectorValidator.build(
    ...
    agent_name="lynx-investor-utilities",
)


def _validate_sector(profile: CompanyProfile) -> None:
    """Raise SectorMismatchError if *profile* is outside the utilities scope."""
...
def run_progressive_analysis(
    ...
    # Validate sector — refuse to analyze non-utilities companies
